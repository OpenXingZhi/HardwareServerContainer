...
** Check Device
Copy [[file:mqtt-io/config-001.yml]] or [[file:mqtt-io/config-002.yml]] to [[file:mqtt-io/config.yml]], then edit and verify =vid=, =pid=, and other device values against =lsusb=.

#+begin_src sh
  lsusb
#+end_src

Runtime config is [[file:mqtt-io/config.yml]].
